...
1. **Security**: Ensure the Virtual Machines are only accessible via secure ports (RDP/SSH) while blocking all other traffic.
2. **Governance**: Prevent unexpected billing by automatically deallocating all VMs once a specific budget threshold (£2) is reached.
...
1. **Created a Network Security Group (NSG)** and associated it with the VM's subnet.
2. **Defined Inbound Rules**: Configured high-priority rules to allow RDP and SSH traffic from specific IP addresses.
3. **Hardened the Environment**: Verified that all other non-essential ports were blocked to reduce the attack surface.

#### Part 2: The Automated "Kill Switch" (Logic App)
1. **Enabled Managed Identity**: Provided the Logic App with its own identity to bypass authentication errors.
2. **Role Assignments (IAM)**: Granted the Logic App **Virtual Machine Contributor** permissions to the target VMs.
3. **Built the Workflow**: Designed a Logic App to deallocate multiple VMs sequentially upon receiving an alert.
4. **Linked to Budget**: Created a £2 budget alert that triggers an **Action Group**, which in turn calls the Logic App.
